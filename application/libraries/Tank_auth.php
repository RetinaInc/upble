...
class Tank_auth
{
	...
	function is_editor()
	{
		...
		if($this->is_logged_in())
	   {
			return $this->ci->session->userdata('role') == 'editor';
	   }
	   return false;
	...
	* check permission
	*/
	function check($role=null,$only=false)
	{
		$user_role= $this->ci->session->userdata('role');
		$roles=$this->ci->config->item('roles', 'tank_auth');
		if(!$user_role||(!$only&&$roles[$user_role]<$roles[$role])||($only&&$roles[$user_role]!==$roles[$role]))
		$this->denyAccess($user_role);
	
	}
	
	private function denyAccess($user_role)
	{
		if(!$user_role)
		{
			$this->ci->session->set_flashdata('flashMessage','You have to login first!');
			$this->ci->session->set_flashdata('loginRedirect',$this->ci->uri->uri_string());
			redirect('/ucp/login/');
			
		}
		$this->ci->session->set_flashdata('flashMessage','Permission Denied');
		$redirect=isset($_SERVER['HTTP_REFERER'])?$_SERVER['HTTP_REFERER']:'';
		redirect($redirect);
	}
	
	//get new message count
	function get_newpm_count()
	{
		$count=0;
		if($this->is_logged_in())
		{
			$count=$this->ci->users->get_pm_count($this->get_user_id());
		}
		return $count;
	}
	...
	function get_user_city()
	{
		...
		$this->ci->load->helper('cookie');
		if($cookie_v = get_cookie($this->ci->config->item('default_city_cookie_name'),TRUE))
		{
...
// 	    if(!$city)
// 	    	exit('This website seems not properly installed!');
		$this->city = $city;
		return $city;
	}
	...
	function set_user_city($city)
	{
		...
		$this->ci->load->helper('cookie');
		$cookie = array(
		'name' => $this->ci->config->item('default_city_cookie_name'),
		'value' => $city->slug,
		'expire' => 365*24*60*60
	...
	function get_cities()
	{
		...
		$cities = $this->ci->cc->get_top();
		if($this->city)
			unset($cities[$this->city->id]);
		return $cities;
	...
	{
		$this->ci->load->model('catsAndCities','cc');
		$this->ci->cc->set_table_name('category');
		return $this->ci->cc->get_top();
	}
